...
合并到 `main` 会触发站点部署。应用版本和数据补丁分别使用 `vX.Y.Z` 与 `patch-X.Y.Z` 标签；两种标签的校验、artifact 和 GitHub Release 行为不同。数据发布依赖匹配补丁版本的 SQLite 库，完整步骤及仓库当前限制见[发布与部署说明](docs/DEPLOYMENT.md)。
